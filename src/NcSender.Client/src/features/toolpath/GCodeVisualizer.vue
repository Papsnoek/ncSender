...
<template>
  <section class="viewport">
    ...
    <div ref="canvas" class="viewport__canvas" @contextmenu="handleContextMenu">
      ...
      <div class="floating-toolbar floating-toolbar--top">
        ...
        <div class="file-controls">
          <input
            ref="fileInput"
            type="file"
            accept=".nc,.gcode,.gc,.ngc,.tap,.txt"
      ...
      <div class="coolant-controls" :class="{ 'controls-disabled': isCoolantDisabled }">
        <div
      ...
      <div class="control-buttons" :class="{ 'controls-disabled': !store.isConnected.value || (store.homingCycle.value > 0 && !store.isHomed.value) || store.isProbing.value }">
        ...
        >
          <svg class="btn-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M6 4l14 8-14 8V4z"/></svg>
          {{ isOnHold ? 'Resume' : 'Cycle' }}
        </button>
        <!-- Pause: shown when job is active (running/hold) -->
        <button
          v-if="isJobActive"
          ...
          title="Pause Job"
        >
          <svg class="btn-icon" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="4" width="4" height="16" rx="1"/><rect x="14" y="4" width="4" height="16" rx="1"/></svg>
          Pause
        </button>
        <button
        ...
        <!-- From Line: shown when no job is active -->
        <button
          v-if="!isJobActive"
...
const overrideControls = useOverrideControls();
...
// Machine-centric bounds - fixed machine limits without WCO offset
// Used for out-of-bounds checking since toolpaths are now at machine coordinates
const computeMachineBounds = () => {
  const gridSizeX = resolveGridSize(props.gridSizeX);
  const gridSizeY = resolveGridSize(props.gridSizeY);
  const orientation = resolvedOrientation.value;

  // Fixed machine limits (no offset)
  const xBounds = computeAxisBounds(gridSizeX, 0, orientation.xHome);
  const yBounds = computeAxisBounds(gridSizeY, 0, orientation.yHome);

  const zMax = resolveZTravel(props.zMaxTravel);
  const zBounds = zMax != null ? computeAxisBounds(zMax, 0, orientation.zHome) : null;
...
const applyBoundsAndWarnings = () => {
  ...
  // Toolpaths are now at machine coordinates, so check against machine limits
  const machineBounds = computeMachineBounds();
  gcodeVisualizer.setGridBounds(machineBounds);
...
const rebuildGrid = (_workOffset = props.workOffset, viewType: 'top' | 'front' | 'iso' | 'split' = props.view, skipCameraFit = false) => {
  ...
  // Rebuild axis labels at new WCO position
  if (scene && axisLabelsGroup) {
    scene.remove(axisLabelsGroup);
    axisLabelsGroup = createDynamicAxisLabels(wco, viewType === 'split' ? 'top' : viewType);
    scene.add(axisLabelsGroup);
  }
  if (scene && viewType === 'split' && splitSideAxisLabels) {
  ...
  // Update home indicator to match grid Z level
  ...
  // Skip camera fitting when only WCO changed (e.g., during job run)
  if (!skipCameraFit && !autoFitMode.value) {
    if (props.view === 'split') {
      updateSplitViewCameras();
    } else if (spindleViewMode.value) {
      // In spindle view mode, center at origin but keep the same frustum size
      const gridBounds = getGridBounds();
      fitCameraToBounds({ center: new THREE.Vector3(0, 0, 0), size: gridBounds.size });
    } else {
...
const initThreeJS = () => {
  ...
  // Position axes at crosshair (WCO position), on the grid surface
  const initWco = props.workOffset || { x: 0, y: 0, z: 0 };
  axesGroup.position.set(initWco.x, initWco.y, initWco.z + 0.1);
  scene.add(axesGroup);

  // Add initial axis labels (positions are dynamic based on bounds)
  axisLabelsGroup = createDynamicAxisLabels(props.workOffset, props.view === 'split' ? 'top' : props.view);
  scene.add(axisLabelsGroup);

  // For split view, also create axis labels without Y for the side viewport
  if (props.view === 'split') {
    splitSideAxisLabels = createDynamicAxisLabels(props.workOffset, 'front');
    scene.add(splitSideAxisLabels);
  }
  ...
  // Use machine-centric bounds for out-of-bounds detection
  gcodeVisualizer.setGridBounds(computeMachineBounds());
  ...
  // Fit camera to actual grid bounds before the first frame renders (avoids zoom flash)
  if (props.view === 'split') {
    updateSplitViewCameras();
  } else {
...
const setupControls = () => {
  ...
  // Window-level listeners to catch mouse/touch release outside canvas
...
const onMouseDown = (event: MouseEvent) => {
  // Real mouse events (not from touch simulation) should allow click handler
  // Note: Touch events call this with simulated MouseEvent, but we set lastInteractionWasTouch before
  isDragging = true;
  previousMousePosition = { x: event.clientX, y: event.clientY };
  mouseDownPosition = { x: event.clientX, y: event.clientY };

  // Track right-click for context menu (quick click vs hold for rotation)
  if (event.button === 2) {
...
const onMouseUp = (event: MouseEvent | Event) => {
  // Check for quick right-click release to show context menu
  if (event instanceof MouseEvent && event.button === 2) {
    ...
    if (clickDuration < 300 && isValidView && !isJobRunning.value) {
      showContextMenuAt(rightClickStartPos.x, rightClickStartPos.y);
    }
  }
...
const animate = () => {
  ...
  // Update execution distance every frame for smooth progressive graying
  if (gcodeVisualizer?.executionActive && lastExecutedLine.value > 0) {
    const ln = lastExecutedLine.value;
...
// Transform context menu handlers - just prevent native menu, actual menu shown on mouseup
const handleContextMenu = (event: MouseEvent) => {
  // Prevent native context menu in views where we have custom menu
  if (props.view === 'top' || props.view === 'front' || props.view === 'iso') {
    event.preventDefault();
  }
};
...
const applyTransform = async (
  ...
  try {
    // Get source file info - only preserve sourceFile if it actually exists (real file from gcode-files)
    ...
    const baseName = nameForTransform.replace(/\.[^.]+$/, '');
    const ext = nameForTransform.match(/\.[^.]+$/)?.[0] || '.nc';
...
const handleGCodeUpdate = async (data: { filename: string; content?: string; timestamp: string }) => {
  try {
    ...
    // Set WCO for machine-centric rendering
    if (props.workOffset) {
    ...
    // Update axis labels at crosshair position
    if (axisLabelsGroup) {
      scene.remove(axisLabelsGroup);
    }
    axisLabelsGroup = createDynamicAxisLabels(props.workOffset, props.view);
    scene.add(axisLabelsGroup);
    updatePointerScale();
    updateAxisLabelsScale();
...
const updatePointerType = () => {
  if (cuttingPointer && scene) {
    scene.remove(cuttingPointer);
    cuttingPointer.traverse((child: any) => {
...
const getGridBounds = () => {
  // Machine-centric: grid bounds are fixed based on machine limits only
  // Z level is at WCO.z (work surface level)
  const gridSizeX = resolveGridSize(props.gridSizeX);
  const gridSizeY = resolveGridSize(props.gridSizeY);
  const orientation = resolvedOrientation.value;
  const wcoZ = props.workOffset?.z || 0;

  // Calculate fixed machine bounds (same logic as machineBounds in helpers.js)
  let minX, maxX, minY, maxY;
  if (orientation.xHome === 'max') {
    minX = -gridSizeX;
    maxX = 0;
  } else {
    minX = 0;
...
const handleViewButtonClick = async (viewType: 'top' | 'front' | 'iso' | 'split') => {
  ...
  if (viewType === 'split') {
    ...
    if (spindleViewMode.value) {
      // In spindle view mode, center at origin
      // Use gcode bounds size if autoFit is ON, otherwise use grid bounds size
      const bounds = (autoFitMode.value && currentGCodeBounds) ? currentGCodeBounds : getGridBounds();
      fitCameraToBounds({ center: new THREE.Vector3(0, 0, 0), size: bounds.size }, viewType);
    } else if (autoFitMode.value && currentGCodeBounds) {
...
const updateSplitViewCameras = () => {
  ...
  // Get bounds to fit - respect autoFitMode and spindleViewMode settings
  let bounds = (autoFitMode.value && currentGCodeBounds) ? currentGCodeBounds : getGridBounds();
  if (!bounds) return;

  // In spindle view mode, center at origin but keep the same size
  if (spindleViewMode.value) {
    bounds = {
...
watch(() => props.view, (newView) => {
  if (newView === 'split') {
    ...
    if (scene) {
      ...
      axisLabelsGroup = createDynamicAxisLabels(props.workOffset, 'top');
      scene.add(axisLabelsGroup);
    ...
    if (scene && axisLabelsGroup) {
      scene.remove(axisLabelsGroup);
      axisLabelsGroup = createDynamicAxisLabels(props.workOffset, newView);
      scene.add(axisLabelsGroup);
    }
    ...
    if (spindleViewMode.value) {
      ...
      // Use gcode bounds size if autoFit is ON, otherwise use grid bounds size
      const bounds = (autoFitMode.value && currentGCodeBounds) ? currentGCodeBounds : getGridBounds();
      fitCameraToBounds({ center: new THREE.Vector3(0, 0, 0), size: bounds.size }, newView);
    } else if (autoFitMode.value && currentGCodeBounds) {
...
// Machine-centric: spindle is positioned at MPos (machine coordinates), not WPos
// Use JSON.stringify for reliable change detection since object is updated in place
let lastMachineCoordsJson = '';
let spindleInitialized = false;
watch(
  () => props.machineCoords ? `${props.machineCoords.x},${props.machineCoords.y},${props.machineCoords.z}` : '',
  (newJson) => {
    if (!newJson || newJson === lastMachineCoordsJson) return;
    lastMachineCoordsJson = newJson;

    const coords = props.machineCoords;
    if (coords) {
      // Spindle position is at machine coordinates (MPos)
      // which matches the rendered paths (work coords + WCO)
      targetSpindlePosition.x = coords.x;
      targetSpindlePosition.y = coords.y;
      targetSpindlePosition.z = coords.z;

      // Initialize current position on first coordinate update
      if (!spindleInitialized) {
...
// Track last WCO to avoid unnecessary grid rebuilds (WCO is reported on every status update)
let lastWCO = { x: 0, y: 0, z: 0 };

// Watch for work offset (WCO) changes to update the grid (crosshair position)
// WCO changes when G92, tool length offset, or active workspace changes
// NOTE: We do NOT re-render G-code here - parts stay at fixed machine positions
watch(() => props.workOffset, (newOffset) => {
  ...
  // Only process if WCO actually changed (avoid rebuilding grid on every status update)
  const wcoChanged = !newOffset ||
    Math.abs((newOffset.x || 0) - lastWCO.x) > 0.001 ||
    Math.abs((newOffset.y || 0) - lastWCO.y) > 0.001 ||
    Math.abs((newOffset.z || 0) - lastWCO.z) > 0.001;

  if (!wcoChanged) return;

  // Update last known WCO
  if (newOffset) {
...
watch(() => autoFitMode.value, async (isAutoFit) => {
  ...
  if (props.view === 'split') {
    ...
    if (spindleViewMode.value) {
      // In spindle view mode, center at origin
      // Use gcode bounds size if autoFit is ON, otherwise use grid bounds size
      const bounds = (isAutoFit && currentGCodeBounds) ? currentGCodeBounds : getGridBounds();
      fitCameraToBounds({ center: new THREE.Vector3(0, 0, 0), size: bounds.size });
    } else if (isAutoFit && currentGCodeBounds) {
...
onMounted(async () => {
  ...
  // Watch for jobLoaded changes to handle clearing and loading
  watch(() => props.jobLoaded?.filename, async (newValue, oldValue) => {
    if (oldValue && !newValue) {
      // Program was cleared
      handleGCodeClear();
    } else if (newValue && !hasFile.value) {
      // Filename available but visualizer empty - load from IDB
      try {
        const { getGCodeFromIDB } = await import('../../lib/gcode-store.js');
        const gcodeData = await getGCodeFromIDB();
        if (gcodeData?.content) {
  ...
  setTimeout(() => {
    ...
    if (props.view === 'split') {
      ...
      if (spindleViewMode.value) {
        ...
        // Use gcode bounds size if autoFit is ON, otherwise use grid bounds size
        const bounds = (autoFitMode.value && currentGCodeBounds) ? currentGCodeBounds : getGridBounds();
        fitCameraToBounds({ center: new THREE.Vector3(0, 0, 0), size: bounds.size }, props.view);
      } else if (autoFitMode.value && currentGCodeBounds) {
...
watch(
  ...
  (newVal, oldVal) => {
    ...
    // Activate execution mode when job starts running and line advances
    const jobStatus = props.jobLoaded?.status;
    if (jobStatus === 'running' && next > 0 && !gcodeVisualizer.executionActive && gcodeVisualizer.lineDistanceRange.size > 0) {
      gcodeVisualizer.setExecutionActive(true);
    }
...
/* Override Controls - bottom center above control buttons */
.override-controls {
  position: absolute;
  bottom: 92px;
  left: 50%;
